...
class TestService1(TestBase):
    ...
    def test_service1(self):
        with requests_mock.Mocker() as mocker:
            mocker.get("http://service_2_api:5001/city", text='Milan')
            mocker.get("http://service_3_api:5002/activity", text='Surfing')
            mocker.post("http://service_4_api:5003/price", text='400') 
            response = self.client.get(url_for('index'))
            self.assertEqual(response.status_code, 200)
